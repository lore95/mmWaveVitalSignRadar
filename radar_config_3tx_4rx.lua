...
-- raised from 26.19 -> 40: must satisfy RAMP_END_TIME >= ADC_START_TIME + ADC_SAMPLES/SAMPLE_RATE.
-- At 256 samples / 10000 ksps that's 7.07 + 25.6 = 32.67us minimum; 40us gives margin.
-- Re-check in RampTimingCalculator before flashing if you change ADC_SAMPLES/SAMPLE_RATE again.
RAMP_END_TIME = 40 -- us
...
-- raised from 64 -> 256: 64 samples gave ~18cm range bins, too coarse to isolate
-- the chest in its own bin cleanly against clutter. 256 keeps RAMP_END_TIME budget
-- sane while giving much finer range resolution.
ADC_SAMPLES = 256
-- SAMPLE_RATE: sample rate for ADC_SAMPLES, changing this along with ADC_SAMPLES and 
-- FREQ_SLOPE will affect the bandwidth
SAMPLE_RATE = 10000 -- ksps (matches the working tx0/rx0 capture's 10 Msps)
...
-- raised from 1 -> 64: with CHIRP_LOOPS=1 there was nothing to coherently
-- average per TX per frame, which is what the tx0/rx0 pipeline relies on to
-- pull the chest's sub-millimeter phase modulation out of the noise floor.
CHIRP_LOOPS = 64
...
-- LVDSLaneConfig: all 4 physical lanes enabled. With 4 RX active and complex(2x)
-- sampling, each RX channel needs its own lane — 2 lanes (the original setting)
-- would only carry data for 2 of your 4 RX channels.
