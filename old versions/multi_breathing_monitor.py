...
CONFIRM_FRAMES    = 3
DELETE_FRAMES     = 50
MIN_PEAK_SEP_BINS = 10
MIN_TRACK_SEP_BINS = 8
...
def separate_tx(frame: np.ndarray):
    return frame[0::2], frame[1::2]  # TX0, TX2 each (64, 4, 540)
...
def compute_range_angle_map(virtual: np.ndarray, n_fft: int = AOA_FFT_SIZE) -> np.ndarray:
    ...
    Args:
        virtual: (8, n_bins) complex — bg-subtracted virtual array
...
def estimate_aoa(rfft_tx0: np.ndarray, rfft_tx2: np.ndarray,
    ...
    virtual = np.zeros(NUM_VIRTUAL, dtype=np.complex64)
...
class DemoSource(threading.Thread):
    ...
    def run(self):
        ...
        print(f"[DEMO] 2TX/4RX MIMO — 2 targets:")
        ...
        while self.running:
            ...
            frame_iq = np.zeros((TOTAL_CHIRPS, NUM_RX, NUM_ADC_SAMPLES),
                                dtype=np.complex64)
            for loop in range(NUM_CHIRPS):
                for tx in range(NUM_TX):
                    ci = loop * NUM_TX + tx
                    tx_off = tx * 4
                    for rx in range(NUM_RX):
                        vi = tx_off + rx
                        s1 = 2*np.pi * vp[vi] * 0.5 * np.sin(a1)
                        s2 = 2*np.pi * vp[vi] * 0.5 * np.sin(a2)
            ...
            raw[0::4] = np.round(flat.real).astype(np.int16)
            raw[1::4] = 0
            raw[2::4] = np.round(flat.imag).astype(np.int16)
            raw[3::4] = 0
            with self.lock:
...
def main():
    ap = argparse.ArgumentParser(description="Multi-target breathing monitor (2TX/4RX MIMO)")
    ...
    frame_count = 0
    bg_frames_buf = []       # list of (8, n_bins) virtual arrays for calibration
    bg_virtual_saved = None  # (8, n_bins) complex background
    ...
    if radar_enabled:
        ...
        ax_ra.set_title("radar monitor  (2TX/4RX MIMO)", color="white", fontsize=11)
        ...
        # Tracked target scatter overlay (in cartesian coords)
    ...
    def update(_):
        ...
        if radar_enabled:
            ...
            for raw_chunk in frames_this_tick:
                ...
                tx0_chirps, tx2_chirps = separate_tx(frame)
                ...
                # Full virtual array: (8, n_bins)
                virtual = np.concatenate([rfft_tx0, rfft_tx2], axis=0)
                ...
                if bg_virtual_saved is None:
                    ...
                    bg_virtual_saved = np.mean(all_v, axis=0)  # (8, n_bins)
                    ...
                    print(f"[RADAR] background done ({BG_FRAMES} frames, "
                          f"8 virtual elements)")

                # Background subtraction on full virtual array
                diff_virtual = virtual - bg_virtual_saved  # (8, n_bins)

                # 1D range profile for peak detection
                ...
                # 2D range-angle map
                ra_full = compute_range_angle_map(diff_virtual)  # (AOA_FFT_SIZE, n_bins)
            ...
            if track_mgr is not None:
                ...
                # Scatter overlay: convert (range, angle) to (x, y) cartesian
                if n_conf > 0:
                    sc_ranges = [t.range_m for t in confirmed]
                    sc_angles_deg = [track_angles.get(t.track_id, 0) for t in confirmed]
                    sc_x = [r * np.sin(np.radians(a)) for r, a in zip(sc_ranges, sc_angles_deg)]
                    sc_y = [r * np.cos(np.radians(a)) for r, a in zip(sc_ranges, sc_angles_deg)]
                    sc_c = [t.color for t in confirmed]
                    scatter_tracks.set_offsets(np.column_stack([sc_x, sc_y]))
                    scatter_tracks.set_facecolors(sc_c)
                    scatter_tracks.set_edgecolors("white")
                else:
                ...
                if txt_status:
                    if n_conf == 0:
                        ...
                        for t in confirmed:
                            ...
                            parts.append(f"{t.range_m:.1f}m/{a:+.0f}°")
                        txt_status.set_text(f"tracking {n_conf}: {', '.join(parts)}")
                ...
                for i in range(MAX_TRACKS):
                    if i < n_conf:
                        ...
                        bpm_texts[i].set_color("#444444")

                # Displacement waveforms
                latest_t = 0
                any_data = False
                for i in range(MAX_TRACKS):
                    if i < n_conf and len(confirmed[i].disp_hist) > 1:
                        t = confirmed[i]
